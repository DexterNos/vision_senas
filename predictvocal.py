...
modelo = r"C:\Users\jonba\Documents\Maestria\Vocales y audio\Modelo_vocales_colab.h5"
peso =  r"C:\Users\jonba\Documents\Maestria\Vocales y audio\Pesos_vocales_colab.h5"
cnn = load_model(modelo)  #Cargamos el modelo
cnn.load_weights(peso)  #Cargamos los pesos

direccion = r"C:\Users\jonba\Documents\Maestria\Vocales y audio\Validacion vocales\Validacion Colab"
...
while (1):
    ...
    if resultado.multi_hand_landmarks: #Si hay algo en los resultados entramos al if
        for mano in resultado.multi_hand_landmarks:  #Buscamos la mano dentro de la lista de manos que nos da el descriptor
            for idx, lm in enumerate(mano.landmark):  #Vamos a obtener la informacion de cada mano encontrada por el ID
